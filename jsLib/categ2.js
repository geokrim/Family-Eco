function getAllCateg2IntoTable() {
  $.ajax({
    url: serverURL + "/categ2/all",
    dataType: "json",
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      alert("Error loading categ2: " + textStatus + " - " + errorThrown);
    },
  });
}

function showCateg2(id) {
  $.ajax({
    url: serverURL + "/categ2/" + id,
    dataType: "json",
    success: function (data) {
      $("#id").val(data.id);
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      alert("Error showing categ2: " + textStatus + " - " + errorThrown);
    },
  });
}

function saveUpdateCateg2() {
  ...
  const url = isNew
    ? serverURL + "/categ2/add"
    : serverURL + "/categ2/updateCateg2/" + categData.id;
...
function deleteCateg2(id) {
  if (!confirm("Delete this category?")) return;
  $.ajax({
    url: serverURL + "/categ2/canDelCateg2/" + id,
    type: "GET",
    success: function (response) {
      if (response === "YES") {
        $.ajax({
          url: serverURL + "/categ2/delCateg2/" + id,
          type: "GET",
          success: function () {
            alert("Category 2 deleted.");
            getAllCateg2IntoTable();
          },
          error: function (jqXHR, textStatus, errorThrown) {
